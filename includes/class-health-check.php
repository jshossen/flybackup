...
class Fly_Backup_Health_Check {
    ...
    private function register_checks() {
        ...
        $this->checks = apply_filters('flybackup_health_checks', $this->checks);
    }
    ...
    public function check_disk_space() {
        ...
        if ($available_space === false) {
            ...
            $status = 'critical';
            $message = 'Low disk space: ' . flybackup_format_bytes($available_space) . ' available';
        } else {
            $message = flybackup_format_bytes($available_space) . ' available';
        }
    ...
    public function check_memory_limit() {
        $memory_limit = flybackup_get_php_memory_limit();
        ...
        if ($memory_limit < $recommended_limit) {
            $status = 'warning';
            $message = 'Memory limit is low: ' . flybackup_format_bytes($memory_limit);
        } else {
            $message = 'Memory limit: ' . flybackup_format_bytes($memory_limit);
        }
