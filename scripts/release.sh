...
# Get version from flybackup.php (plugin header)
VERSION=$(grep -oE 'Version:[[:space:]]+[0-9.]+' "$ROOT_DIR/flybackup.php" | head -n1 | sed -E 's/Version:[[:space:]]+//')
[[ -n "$VERSION" ]] || abort "Could not extract version from flybackup.php"
...
# Staging directory for clean copy (ZIP extracts at root level)
...
# Ensure zip exists
[[ -f "$ZIP_PATH" ]] || abort "ZIP not found: $ZIP_PATH"
echo "${LOG_PREFIX} found distributable zip at $ZIP_PATH"

# Unzip into staging directory for SVN sync
mkdir -p "$STAGE_DIR"
unzip -q "$ZIP_PATH" -d "$RELEASE_DIR/stage"
echo "${LOG_PREFIX} staged clean copy at $STAGE_DIR"

if [[ $SKIP_DEPLOY -eq 0 ]]; then
  echo "${LOG_PREFIX} deploying to WordPress SVN"
  svn checkout "$SVN_REPO_URL" "$SVN_DIR" "${SVN_AUTH_ARGS[@]}"
  rsync -av --delete "$STAGE_DIR/" "$SVN_DIR/trunk/"
